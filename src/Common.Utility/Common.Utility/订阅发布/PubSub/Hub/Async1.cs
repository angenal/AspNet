...
namespace PubSub.Hub
{
    ...
    public class Async1<T> : IDisposable
    {
        /// <summary>
        /// Subscribe Tasks
        /// </summary>
        ...
        private static Async1<T> _default;

        public static Async1<T> Default => _default ?? (_default = new Async1<T>(TimeSpan.Zero, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(30)).Start());

        public Async1(TimeSpan onceInterval, TimeSpan interval, TimeSpan timeout, int onceConcurrentTasks = 0)
        {
