...
namespace PubSub.Hub
{
    ...
    public class Async<T> : IDisposable
    {
        /// <summary>
        /// Subscribe Tasks
        /// </summary>
        ...
        private static Async<T> _default;
        private static Async<T> _defaultBulk;

        public static Async<T> Default => _default ?? (_default = new Async<T>(TimeSpan.Zero, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(30)).Start());
        public static Async<T> DefaultBulk => _defaultBulk ?? (_defaultBulk = new Async<T>(TimeSpan.Zero, TimeSpan.FromSeconds(1), TimeSpan.FromHours(1), -1).Start());

        public Async(TimeSpan onceInterval, int onceConcurrentTasks = 0) : this(onceInterval, TimeSpan.Zero, TimeSpan.FromSeconds(30), onceConcurrentTasks)
        { }
        public Async(TimeSpan onceInterval, TimeSpan timeout, int onceConcurrentTasks = 0) : this(onceInterval, TimeSpan.Zero, timeout, onceConcurrentTasks)
        { }
        public Async(TimeSpan onceInterval, TimeSpan interval, TimeSpan timeout, int onceConcurrentTasks = 0)
        {
            ...
            while (_started)
            {
                ...
                try
                {
                    while (Handlers.Count > 0 && _timeStampSeed < _timeStamp)
                    {
                        ...
                        while (hasValue)
                        {
                            if (_onceConcurrentTasks > 0)
                            {
                                var list = new List<T>();
                                for (var i = 0; i < _onceConcurrentTasks && items.TryTake(out var item); i++)
                                    list.Add(item);
        ...
        {
            //LogFactory.GetLogger("Info").Info(s);
        }
    }
}
