...
namespace PubSub.Hub
{
    ...
    public class Async3<T> : IDisposable
    {
        /// <summary>
        /// Subscribe Tasks
        /// </summary>
        ...
        private static Async3<T> _default;

        public static Async3<T> Default => _default ?? (_default = new Async3<T>(TimeSpan.Zero, TimeSpan.FromSeconds(1), TimeSpan.FromHours(1), -1).Start());

        public Async3(TimeSpan onceInterval, TimeSpan interval, TimeSpan timeout, int onceConcurrentTasks = 0)
        {
