...
namespace Common.Utility
{
    ...
    public static class DateTimeUtility
    {
        ...
        public static DateTime? ConvertObjectToNullableDate(object obj)
        {
            ...
            if (DateTime.TryParse(obj.ToString(), out DateTime tmpDateTime) == true)
                return tmpDateTime;
            else
                return null;
        ...
        /// 中文星期名称
        /// </summary>
        private static string[] arrWeekDayName = { "星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六" };
        /// <summary>
        /// 获取星期几的中文名称
        /// </summary>
        /// <param name="dt"></param>
        ...
        {
            int wd1 = (int)weekday, wd2 = (int)dt.DayOfWeek;
        ...
        /// <returns></returns>
        public static int WeekOfMonth(DateTime dt, int WeekStart)
        {
            //第一天
            DateTime firstDay = new DateTime(dt.Year, dt.Month, 1);
            int i = (int)firstDay.DayOfWeek;
            if (i == 0) i = 7;
            if (WeekStart == 1)
        ...
        {
            //SET @WM = datediff(week, dateadd(week, datediff(week, 0, dateadd(month, datediff(month, 0, @OneDate), 0)), 0), @OneDate - 1) + 1  -- WeekOfMonth
            //IF @WM = 0 SET @M = @M - 1; IF @M = 0 SET @M = 1; --Month Repair
            var OneDate = time.Date;



            var firstDayOfMonth = new DateTime(time.Year, time.Month, 1);
            var i = (int)firstDayOfMonth.Date.DayOfWeek;
            if (i == 0) i = 7;

            // 周一至周日 为一周
            var week = (time.Date.Day + i - 2) / 7 + 1;
            // 周日至周六 为一周
            //week = (day.Date.Day + i - 1) / 7;
            ...
            // 当天日期[月末时:周一周二]
            if ((time.AddDays(1).Month > time.Month && time.DayOfWeek == DayOfWeek.Tuesday)
                || (time.AddDays(2).Month > time.Month && time.DayOfWeek == DayOfWeek.Monday))
            {
                week = 0;//计算至下个月
            }
            // 处理特殊的当年最后两天
            if (week > 4 && time.Month == 12 &&
                ((time.AddDays(1).Year > time.Year && time.DayOfWeek == DayOfWeek.Tuesday)
                 || (time.AddDays(2).Year > time.Year && time.DayOfWeek == DayOfWeek.Monday)))
            {
                week = 5;//当年最后一周
            }
        ...
        //    //第一个周日
        //    int firstSunday = (7 - (int)firstDay.DayOfWeek) % 7 + 1;
        //    if (dt.DayOfYear < firstSunday)
        //        return 1;
        //    else if (firstSunday == 1)
        //        return (dt.DayOfYear - firstSunday) / 7 + 1;
        //    else
        //        return (dt.DayOfYear - firstSunday) / 7 + 2;
        //}
