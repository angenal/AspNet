...
namespace Common.Utility
{
    ...
    public static class StringHelper
    {
        ...
        /// <returns></returns>
        public static string ChineseUrlEncode(this string url)
        {
            if (string.IsNullOrWhiteSpace(url)) return url;

            string s1 = "", s2 = "", s3 = "";
            int i = url.LastIndexOf('/'); if (i > 0) { s1 = url.Substring(0, i + 1); }
            int j = url.LastIndexOf('.'); if (j > i) { s2 = url.Substring(i + 1, j - i - 1); if (url.Length > j) s3 = url.Substring(j); } else if (i > 0) { s2 = url.Substring(i + 1); }
            return s1.Length > 0 && s2.Length > 0 ? s1 + HttpUtility.UrlEncode(s2).Replace("+", "%20") + s3 : url;
        }
        #endregion
