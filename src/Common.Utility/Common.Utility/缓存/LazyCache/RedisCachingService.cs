...
namespace LazyCache
{
    ...
    public class RedisCachingService : IAppCache, IDisposable
    {
        private readonly Lazy<ICacheProvider> cacheProvider;

        private readonly SemaphoreSlim locker = new SemaphoreSlim(1, 1);

        public RedisCachingService() : this(DefaultCacheProvider)
        {
        }

        public RedisCachingService(Lazy<ICacheProvider> cacheProvider)
        {
            this.cacheProvider = cacheProvider;
        }

        public RedisCachingService(Func<ICacheProvider> cacheProviderFactory)
        {
            cacheProvider = new Lazy<ICacheProvider>(cacheProviderFactory);
        }

        public RedisCachingService(ICacheProvider cache) : this(() => cache)
        {
        }

        public static Lazy<ICacheProvider> DefaultCacheProvider { get; set; }
            = new Lazy<ICacheProvider>(() => new RedisCacheProvider());
        ...
        public virtual void Set<T>(string key, T item, DateTime dependencyTimestamp)
        {
            ...
            CacheProvider.Set(key + dependencyTimestamp.Ticks, item, DefaultConfiguration.DefaultDependency);
        }
        ...
        public virtual T Get<T>(string key)
        {
            ...
            var item = CacheProvider.Get<T>(key);
        ...
        public virtual Task<T> GetAsync<T>(string key, DateTime dependencyTimestamp)
        {
            return GetAsync<T>(key + dependencyTimestamp.Ticks);
        }
        ...
        public virtual T GetOrAdd<T>(string key, Func<ICacheEntry, T> addItemFactory, MemoryCacheEntryOptions policy)
        {
            ...
            locker.Wait(); //TO.DO: do we really need this? Could we just lock on the key?
            try
            {
        ...
        public virtual async Task<T> GetOrAddAsync<T>(string key, Func<ICacheEntry, Task<T>> addItemFactory, MemoryCacheEntryOptions policy)
        {
            ...
            await locker.WaitAsync()
                .ConfigureAwait(
                    false); //TO.DO: do we really need to lock everything here - faster if we could lock on just the key?
