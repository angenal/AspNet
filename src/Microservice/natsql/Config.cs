...
namespace natsql
{
    ...
    public class Config : ICloneable
    {
        ...
        public int Init(long created, int loadInterval, bool reload = false)
        {
            ...
            // enable debug
            Db.Debug = Log != null && "debug" == Log.Level;
            ...
            if (IsJsTemplate())
            {
                ...
                var id = V8Script.Extensions.Crc32(jsContent);
            ...
            if (IsDbTemplate())
            {
                ...
                if (Nats.Script.EndsWith(".db"))
                {
                    ...
                    Util.SQLite.ConnectionString = $"Data Source={dbPath};Mode=ReadOnly"; // Data Source=natsql.db;Password=123456
                    dt = Util.SQLite.Query($"Select * From {Nats.Table} Where Deleted=0 and Created>{created}");
                }
                else
                {
                    // connect other db
        ...
        string InsertItems(Subscribes data)
        {
            ...
            if (IsDbTemplate())
            {
                ...
                if (data.Created <= 0) data.Created = V8Script.TimeExt.ToTimestampSeconds(DateTime.Now);
