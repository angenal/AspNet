# Nats远程RPC+Redis队列+Javascript调度运行

> 1.订阅任务natsql.js || 数据库存储订阅任务，请参考: `natsql.mssql.sql` `natsql.mysql.sql` `natsql.sqlite.sql`
~~~javascript
subscribe = [
    {
...
CREATE TABLE Subscribes (
	Id varchar(36) NOT NULL PRIMARY KEY CLUSTERED, -- 主键ID
	Name varchar(100) NOT NULL, -- subscribe.name 订阅任务名
	Spec varchar(2) NOT NULL,   -- subscribe.spec 特殊表达式
	Func varchar(100) NOT NULL, -- subscribe.func 文件目录名
	Content text NOT NULL,      -- 执行任务natsql.js 脚本代码 > function sql(records)
	CacheDir varchar(100),      -- subscribe.func 缓存目录名
	MsgLimit int NOT NULL,      -- 100000000  消息总量限制
	BytesLimit int NOT NULL,    -- 1024  每个消息字节数限制
	Amount int NOT NULL,        -- 0   # 批量获取记录数限制
	Bulk int NOT NULL,          -- 100 # 批量插入记录数<=2000
	Interval int NOT NULL,      -- 2000 # 间隔2000毫秒,批量处理一次
	Version int NOT NULL DEFAULT ((1)), # 跟踪版本号
	Created bigint NOT NULL DEFAULT (datediff(second,'1970-01-01 00:00:00',getdate())), 
	Deleted bit NOT NULL DEFAULT ((0))  # 跟踪状态(0:正常,1:删除)
) ON PRIMARY;
CREATE TRIGGER Trigger_Update_Subscribes ON Subscribes AFTER UPDATE AS BEGIN
	SET NOCOUNT ON;
	UPDATE Subscribes SET Version = Version + 1 WHERE Id IN (SELECT Id FROM inserted);
END;
~~~

> 2.执行任务natsql.js 脚本代码 > function sql(records) > 与 subscribe.name 一一对应: `./natsql/001` `./natsql/002`
~~~javascript
// 支持的功能列表
