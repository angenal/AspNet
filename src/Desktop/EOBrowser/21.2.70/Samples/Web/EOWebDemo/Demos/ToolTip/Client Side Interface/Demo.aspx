...
<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Demo.aspx.cs" Inherits="Demos_ToolTip_Client_Side_Interface_Demo" %>

<asp:Content ID="Content1" ContentPlaceHolderID="demo" Runat="Server">
    <p>
        This sample demonstrates how to use JavaScript to dynamically update
        a ToolTip control when user moves mouse over a Grid cell. Position mouse
        over any grid cell to activate the tooltip.
    </p>    
    <script type="text/javascript">
//<!--JS_SAMPLE_BEGIN-->
function on_grid_cellover(grid, cell)
{
    //cell maybe null if we are leaving the Grid, or
    //the cell may not have an value. No tooltip for
    //both cases
    if (!cell || !cell.getValue())
        return;

    //Get the client side ToolTip object
    var toolTip = eo_GetObject("<%=ToolTip1.ClientID%>");
    
    //Get the Grid cell text
    var cellText = cell.getValue().toString();
    
    //Update the ToolTip text
    toolTip.setBodyHtml(cellText);

    //Attach the ToolTip to the cell
    toolTip.attach(cell.getDOMElement());
}
//<!--JS_SAMPLE_END-->
    </script>
    <eo:Grid runat="server" id="Grid1" BorderColor="#828790" BorderWidth="1px" GridLines="Both"
        ...
        ClientSideOnCellOver="on_grid_cellover">
        <ItemStyles>
            <eo:GridItemStyleSet>
    ...
    <eo:ToolTip runat="server" ID="ToolTip1"></eo:ToolTip>
</asp:Content>
